use terminal_toys::{smärgs, smargs::{self, List}};

struct MyInput { degrees: usize, images: List<std::path::PathBuf>, }

fn main() {
    let MyInput { degrees, images } = terminal_toys::smärgs!(
      "Rotate a bunch of image files by degrees (divisable by 90)",
      MyInput {
        degrees:("Degrees to rotate clockwise", vec!["deg"], smargs::Kind::Required),
